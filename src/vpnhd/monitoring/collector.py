...
class MetricsCollector:
    ...
    async def collect_all_metrics(self) -> None:
        ...
        try:
            ...
            metrics.vpn_errors_total.labels(category="metrics", severity="error").inc()
    ...
    async def _collect_server_metrics(self) -> None:
        ...
        try:
            ...
            result = await execute_command_async(["ip", "link", "show", interface], check=False)
    ...
    async def _count_active_clients(self) -> int:
        ...
        try:
            interface = self.config.get("network.vpn.interface", "wg0")
            result = await execute_command_async(["wg", "show", interface, "peers"], check=False)
    ...
    async def _collect_traffic_metrics(self) -> None:
        ...
        try:
            ...
            result = await execute_command_async(["wg", "show", interface, "transfer"], check=False)
            ...
            for line in lines:
                ...
                metrics.vpn_traffic_received_bytes_total.labels(
                    client_name=client_name, interface=interface
                ).inc(received)
                ...
                if client_name in self._prev_traffic:
                    ...
                    if time_delta > 0:
                        ...
                        metrics.vpn_bandwidth_bytes_per_second.labels(
                            client_name=client_name, interface=interface, direction="rx"
                        ).set(max(0, rx_rate))

                        metrics.vpn_bandwidth_bytes_per_second.labels(
                            client_name=client_name, interface=interface, direction="tx"
                        ).set(max(0, tx_rate))
    ...
    async def _collect_wireguard_metrics(self) -> None:
        ...
        try:
            ...
            result = await execute_command_async(
                ["wg", "show", interface, "latest-handshakes"], check=False
            )
            ...
            # Get endpoints
            result = await execute_command_async(
                ["wg", "show", interface, "endpoints"], check=False
            )
            ...
            for proc in psutil.process_iter(["name", "cpu_percent", "memory_info"]):
                try:
                    if "wireguard" in proc.info["name"].lower() or proc.info["name"] == "wg":
                        metrics.vpn_cpu_usage_percent.labels(process="wireguard").set(
                            proc.info["cpu_percent"] or 0
                        )

                        if proc.info["memory_info"]:
                            metrics.vpn_memory_usage_bytes.labels(process="wireguard").set(
    ...
    async def _collect_disk_metrics(self) -> None:
        ...
        try:
            ...
            if config_path.exists():
                config_size = sum(f.stat().st_size for f in config_path.rglob("*") if f.is_file())
                metrics.vpn_disk_usage_bytes.labels(path=str(config_path), type="config").set(
                    config_size
                )
            ...
            if log_path.exists():
                log_size = sum(f.stat().st_size for f in log_path.rglob("*") if f.is_file())
                metrics.vpn_disk_usage_bytes.labels(path=str(log_path), type="logs").set(log_size)
            ...
            if backup_path:
                ...
                if backup_path.exists():
                    ...
                    metrics.vpn_disk_usage_bytes.labels(path=str(backup_path), type="backups").set(
                        backup_size
                    )
    ...
    async def start(self) -> None:
        ...
        self._running = True
        self.logger.info(f"Starting metrics collector (interval: {self.collection_interval}s)")
    ...
    def record_phase_execution(
        self, phase_name: str, duration: float, status: str = "success"
    ) -> None:
        ...
        """
        metrics.vpn_phase_duration_seconds.labels(phase_name=phase_name).observe(duration)

        metrics.vpn_phase_executions_total.labels(phase_name=phase_name, status=status).inc()

        metrics.vpn_last_phase_execution_timestamp_seconds.labels(phase_name=phase_name).set(
            time.time()
        )

    def record_notification(
        self, channel: str, event_type: str, status: str, duration: Optional[float] = None
    ) -> None:
        ...
        if duration is not None:
            metrics.vpn_notification_duration_seconds.labels(channel=channel).observe(duration)

    def record_ddns_update(
        self, provider: str, record_type: str, status: str, duration: Optional[float] = None
    ) -> None:
        ...
        metrics.ddns_updates_total.labels(
            provider=provider, record_type=record_type, status=status
        ).inc()

        if status == "success":
            metrics.ddns_last_update_timestamp_seconds.labels(
                provider=provider, record_type=record_type
            ).set(time.time())

        if duration is not None:
            metrics.ddns_update_duration_seconds.labels(provider=provider).observe(duration)

    def record_client_connection(self, client_name: str, event_type: str) -> None:
        ...
        metrics.vpn_client_connections_total.labels(
            client_name=client_name, event_type=event_type
        ).inc()
    ...
    def record_error(self, category: str, severity: str = "error") -> None:
        ...
        """
        metrics.vpn_errors_total.labels(category=category, severity=severity).inc()
