...
class KeyRotationManager:
    ...
    def __init__(
        ...
        # Rotation settings
        self.wireguard_interval_days = self.config.get(
            'security.key_rotation.wireguard_days', 90
        )
        self.ssh_interval_days = self.config.get(
            'security.key_rotation.ssh_days', 180
        )
        self.auto_rotation_enabled = self.config.get(
            'security.key_rotation.enabled', False
        )
    ...
    def _load_rotation_history(self) -> None:
        ...
        try:
            ...
            if 'last_ssh_rotation' in history:
                self.last_ssh_rotation = datetime.fromisoformat(
                    history['last_ssh_rotation']
                )
    ...
    async def rotate_wireguard_client_keys(
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                cmd = [
                    ...
                    '-f', str(key_path),
                    '-N', '',  # No passphrase
                    '-C', f'vpnhd-rotated-{datetime.now().isoformat()}'
                ]
    ...
    async def rotate_ssh_keys(self) -> bool:
        ...
        try:
            ...
            if private_key_path.exists():
                ...
                private_key_path.rename(
                    ssh_dir / f'vpnhd_id_ed25519.backup_{backup_suffix}'
                )
                if public_key_path.exists():
                    public_key_path.rename(
                        ssh_dir / f'vpnhd_id_ed25519.pub.backup_{backup_suffix}'
                    )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'wireguard': {
                ...
                'last_rotation': (
                    self.last_wireguard_rotation.isoformat()
                    if self.last_wireguard_rotation else None
                ),
            ...
            'ssh': {
                ...
                'last_rotation': (
                    self.last_ssh_rotation.isoformat()
                    if self.last_ssh_rotation else None
                ),
